...
- **通知栏/锁屏媒体卡**:封面(异步拉取)、曲目/歌手、播放状态,
  动作「喜欢 / 本机静音或恢复 / 下一首 / 离开房间」,经插件 `action` 事件回传前端
...
- **系统中断保护**:来电、其他 App 抢占音频或耳机断开时，只临时静音本机输出；
  房间时间线与 WebSocket 继续推进，恢复声音或回到前台时立即按房间状态校正。
- **弱网重连**:服务端为成员保留 30 秒身份宽限，客户端 WebSocket 重连后自动重发 join，
  避免锁屏网络抖动直接把成员移出房间。
