...
/** {r,g,b} -> "r g b" */
export function toTriplet({ r, g, b }) {
    return `${Math.round(r)} ${Math.round(g)} ${Math.round(b)}`;
}
/** "r g b" -> {r,g,b}，解析失败返回 null */
export function fromTriplet(str) {
 ...
 */
export function quantizeDominant(image) {
    try {
        const size = 32;
 ...
 */
export function darkenForBackground(rgb, targetL = 0.1) {
 ...
 */
export function ensureReadableAccent(rgb) {
