 ...
 */
export default function LyricScroller({ lines, activeIndex, isOwner, onSeek, }) {
    ...
    const focusIndex = lines.length === 0 ? -1 : activeIndex < 0 ? 0 : Math.min(activeIndex, lines.length - 1);
    focusIndexRef.current = focusIndex;
    const prefersReducedMotion = () => typeof window !== 'undefined' &&
        window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const applyOffset = (offsetY, animate) => {
    ...
    const measureAndPin = (animate, index = focusIndexRef.current) => {
        const viewport = viewportRef.current;
        const list = listRef.current;
        if (!viewport || !list || index < 0)
            return;
    ...
    if (lines.length === 0)
        return null;
    return (_jsx("div", { className: "lyric-scroller", ref: viewportRef, "aria-live": "polite", children: _jsx("div", { className: "lyric-list", ref: listRef, children: lines.map((line, index) => {
                const distance = Math.abs(index - focusIndex);
                const role = activeIndex < 0
                    ? distance === 0
                        ? 'near'
                        : 'far'
                    : distance === 0
                ...
                return (_jsxs("button", { type: "button", ref: (el) => {
                    ...
                    }, className: "lyric-item", "data-role": role, "data-focus": isFocus ? 'true' : undefined, onClick: () => {
                        if (!isOwner)
                            return;
                        onSeek(line.time * 1000);
                    }, disabled: !isOwner, title: isOwner
                        ? `跳转到 ${formatClock(line.time)}`
                        : '正在跟随房主', "aria-current": isFocus ? 'true' : undefined, children: [_jsx("span", { className: "lyric-item-text", children: line.text }), line.trans ? (_jsx("span", { className: "lyric-item-trans", children: line.trans })) : null, isOwner ? (_jsx("span", { className: "lyric-item-time", children: formatClock(line.time) })) : null] }, `${line.time}-${index}`));
            }) }) }));
}
