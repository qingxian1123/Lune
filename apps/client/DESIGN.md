...
Windows 音乐库为覆盖在完整播放舞台上的右侧浮层，宽度上限 380px，与工作区边缘保持 16px 留白，避开窗口栏与底部播放条。使用 18px 圆角、深色实底、低透明细边框与柔和环境影；开合不改变封面和歌词的布局。收起后右侧中部保留 44px 悬浮入口，恢复原页签、搜索内容与列表位置。浮层内保持选歌、队列、成员三页签，顶部提供快捷键设置与收起操作；隐藏内容不可被键盘聚焦。

音乐库开合快捷键由本机设置统一管理，支持录入、清除、恢复默认、冲突与保留组合校验，保存后立即生效，入口提示同步更新；只在桌面房间页前台且无弹窗时响应。初次默认展开，之后记住本机开合偏好。开合使用约 200ms 滑动与淡出，并尊重减少动态效果设置。Android 保留底部面板。
