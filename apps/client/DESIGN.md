...
Windows 原生客户端使用 36px 无边框窗口栏，顶部仅保留右侧最小化、最大化/还原、关闭三键，空白区承担拖拽与双击最大化。窗口栏跨首页与房间复用，窗口关闭与离开房间分开。房间邀请码及复制反馈位于成员面板顶部，离开房间位于面板底部并经确认返回首页；窄窗口的音乐库入口位于内容区。浏览器与 Android 不显示原生窗口三键。

Windows 音乐库为右侧浮动面板，宽度在 320–380px 之间，与工作区边缘保持 16px 留白，避开窗口栏与底部播放条。使用 18px 圆角、深色实底、低透明细边框与柔和环境影。工作区宽度达到 800px 时，展开会为音乐库留出安全区，封面与歌词在剩余空间内重新构图；收起后恢复舒展居中的双栏。舞台实际可用宽度不足 720px 时，封面与曲目信息转为歌词上方的紧凑横排。更窄的桌面窗口采用覆盖面板，避免强行压缩阅读区域。收起后右侧中部保留 44px 悬浮入口，恢复原页签、搜索内容与列表位置。浮层内保持选歌、队列、成员三页签，顶部提供快捷键设置与收起操作；隐藏内容不可被键盘聚焦。

音乐库开合快捷键由本机设置统一管理，支持录入、清除、恢复默认、冲突与保留组合校验，保存后立即生效，入口提示同步更新；只在桌面房间页前台且无弹窗时响应。初次默认展开，之后记住本机开合偏好。展开 320ms、收起 260ms，面板与舞台使用同一减速曲线。舞台先完成最终排版，再通过位置补间衔接旧构图：封面允许等比缩放，文字仅平移，避免逐帧换行或拉伸。当前歌词在排版后先重新定位，连续开合从当前可见位置接续；窗口缩放、减少动态效果设置变化与卸载会取消旧动画。Android 保留底部面板。

Windows 分隔线使用两级 1px 细线：窗口栏下沿与播放条上沿使用 14% 柔光象牙；工具页签下沿、邀请码下沿和离开操作上沿使用 8% 柔光象牙，表达内部归组。共享边只绘制一次，进度条位于播放条结构线内侧，不覆盖边界；成员区分隔线与内容统一按 16px 缩进。

### Playback Stage

选歌区的热榜沿用平面列表与薰衣月光：前三名只强调排名文字，行内展示封面、曲目、音源与爱心数，“加入”仅操作队列；移动端加入按钮至少 44px 高。播放区爱心可重复发送，点击产生短暂脉冲，服务端确认后才显示 `+1`，失败显示可读提示，并尊重减少动态效果设置。Windows 使用透明无边框按钮，成功后以薰衣月光填满心形；按服务器、音乐源与歌曲在客户端进程内记忆，切歌返回保留，重新打开客户端清空，不写入本地收藏。鼠标操作无方框，键盘导航保留圆形焦点提示；Android 继续使用不保留点亮状态的线性心形。
