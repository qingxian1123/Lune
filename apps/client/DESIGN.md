...
Windows 分隔线使用两级 1px 细线：窗口栏下沿、侧栏左沿与播放条上沿使用 14% 柔光象牙，形成连续的主结构；工具页签下沿、邀请码下沿和离开操作上沿使用 8% 柔光象牙，表达内部归组。共享边只绘制一次，进度条位于播放条结构线内侧，不覆盖边界；成员区分隔线与内容统一按 16px 缩进，播放舞台保持完整留白。
